...
<body>
  ...
  <!-- Footer -->
  <footer class="footer">
    <p class="footer-text">Generated by Explain Changes MCP · ${new Date().toLocaleString()}</p>
  </footer>
  ...
  <script>
    ...
    // Check if a file diff is a new file (all additions, no deletions)
    ...
    // Check if a file diff is a deleted file (all deletions)
    ...
    function renderDiff(outputFormat) {
      ...
      if (!diffString || diffString.trim() === '') {
        targetElement.innerHTML = '<div style="padding: 40px; text-align: center; color: #8b949e;">No diff content provided</div>';
        return;
      }

      // Split diff by file and render each appropriately
      ...
      fileDiffs.forEach((fileDiff, index) => {
        ...
        // Use unified view for new/deleted files, selected view for modifications
        ...
        try {
          ...
          console.error('diff2html error for file:', err);
          fileContainer.innerHTML = '<div style="padding: 20px; color: #f85149;">Error rendering this file diff</div>';
        }
      });
